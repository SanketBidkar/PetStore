...
def search_product(request):
    if request.method == "GET":
        ...
        if(len(searched_data)==0):
            ...
            # result = Pet.objects.filter(name__icontains=searhed_data)
            query = (Q(name__icontains=searched_data) | Q(species__icontains=searched_data) | Q(breed__icontains=searched_data))
