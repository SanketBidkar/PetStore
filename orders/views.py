...
from .models import Oreders,Payment,OrderPet
...
def place_order(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            data.save()
            order_object = Oreders.objects.get(user=request.user,order_number=orderNumber)


            context = {
                'orders':order_object.pk,
                'cart_items':[item.pk for item in cart_items],
...
def create_order_pdf(order,cart_items):
    ...
    customer_info = [
        ["Custom Name:",order.user.get_full_name()],
...
def send_order_email(order,cart_items):
    ...
    email_body = "Thank You For Placing the Order, Please find the attached invoice for your reference."
...
def payments(request):
    ...
    if request.method == "post":
        try:
            ...
            payment.save()
            last_order_id = Oreders.objects.last().id
            Oreders.objects.filter(id=last_order_id).update(payment_id=payment)

            order_data = Oreders.objects.get(payment_id=payment)
    ...
    if serialized_data:
        ...
        orders = Oreders.objects.get(pk=data['orders'])
        order_number = orders.order_number
        cart_items = Cart.objects.filter(pk__in=data['cart_items'])
